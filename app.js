document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!form) {
    console.error('No se encontró el formulario con id="directorioForm"');
    return;
  }

  if (!aliasesList || !contactosList || !direccionesList || !statusBox) {
    console.error('Faltan contenedores requeridos del formulario.');
    return;
  }
  ...
  function buildPayload() {
    ...
    return {
      id: cleanValue(getValue('id')),
      entidad: {
        id: cleanValue(getValue('id')),
        codigo_interno: cleanValue(getValue('codigo_interno')),
        nombre_comercial: cleanValue(getValue('nombre_comercial')),
        razon_social: cleanValue(getValue('razon_social')),
        rfc: cleanValue(getValue('rfc'))?.toUpperCase() || null,
        tax_id_extranjero: cleanValue(getValue('tax_id_extranjero')),
        regimen_fiscal_id: cleanValue(getValue('regimen_fiscal_id')),
        tipo_persona: cleanValue(getValue('tipo_persona')),
        estatus: cleanValue(getValue('estatus')) || 'prospecto',
        habilitado_operaciones: getChecked('habilitado_operaciones'),
        habilitado_facturacion: getChecked('habilitado_facturacion'),
        habilitado_pagos: getChecked('habilitado_pagos'),
        activo: getChecked('activo'),
        bloqueado_motivo: cleanValue(getValue('bloqueado_motivo')),
        origen: cleanValue(getValue('origen')) || 'manual',
        notas: cleanValue(getValue('notas'))
      },
      roles,
  ...
  function loadDemo() {
    ...
    if (document.getElementById('codigo_interno')) document.getElementById('codigo_interno').value = 'CLI-0001';
    if (document.getElementById('estatus')) document.getElementById('estatus').value = 'activo';
    if (document.getElementById('origen')) document.getElementById('origen').value = 'manual';
    if (document.getElementById('nombre_comercial')) document.getElementById('nombre_comercial').value = 'Baja Forwarding';
    if (document.getElementById('razon_social')) document.getElementById('razon_social').value = 'BAJA FORWARDING, S. DE R.L. DE C.V.';
    if (document.getElementById('rfc')) document.getElementById('rfc').value = 'BFO201127HY3';
    if (document.getElementById('regimen_fiscal_id')) document.getElementById('regimen_fiscal_id').value = '601';
    if (document.getElementById('tipo_persona')) document.getElementById('tipo_persona').value = 'moral';
    if (document.getElementById('habilitado_operaciones')) document.getElementById('habilitado_operaciones').checked = true;
    if (document.getElementById('habilitado_facturacion')) document.getElementById('habilitado_facturacion').checked = true;
    if (document.getElementById('habilitado_pagos')) document.getElementById('habilitado_pagos').checked = true;
    if (document.getElementById('activo')) document.getElementById('activo').checked = true;
    if (document.getElementById('notas')) document.getElementById('notas').value = 'Cuenta demo para pruebas de portal.';
  ...
  if (btnAddAlias) btnAddAlias.addEventListener('click', () => addAlias());

  const btnAddContacto = document.getElementById('btnAddContacto');
  if (btnAddContacto) btnAddContacto.addEventListener('click', () => addContacto());

  const btnAddDireccion = document.getElementById('btnAddDireccion');
  if (btnAddDireccion) btnAddDireccion.addEventListener('click', () => addDireccion());
  ...
  if (btnPreviewBottom) btnPreviewBottom.addEventListener('click', updatePreview);

  const btnLoadDemo = document.getElementById('btnLoadDemo');
  if (btnLoadDemo) btnLoadDemo.addEventListener('click', loadDemo);
